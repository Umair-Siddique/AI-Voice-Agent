...
def _build_whatsapp_system_prompt(tools_list: str) -> str:
    ...
    return (
        ...
        "2. Fetch custom field requirements: Before booking, use available tools to get valid options for location and timezone fields required by the booking system\n"
        "3. Set default values: ALWAYS use Rotterdam as the location and Netherlands as the timezone. Fetch the correct IDs/values for these from the booking system\n"
        "4. Check availability: Use tools to verify the requested time slot is available. Select ANY available therapist - do not ask the customer to choose\n"
        "5. Confirm with client: Present the details (name, date, time at Rotterdam location) and ask 'Should I confirm this booking for you?' DO NOT mention the therapist name\n"
        "6. Execute booking: Only after client confirms, call the booking tool with ALL required fields including additional_fields (Rotterdam location, Netherlands timezone) and any available therapist\n"
        "7. Verify success: Check the tool result carefully. If you get a 400 error about missing fields, fetch the valid options and retry with correct Rotterdam location and Netherlands timezone values\n"
        "8. Send confirmation: If booking succeeded, say something like 'All set! Your appointment is confirmed for [date] at [time] at our Rotterdam location. See you then!' DO NOT mention therapist name\n\n"
        "For RESCHEDULING:\n"
        ...
        "3. Check availability: Verify the new time slot is available at Rotterdam location with any available therapist\n"
        "4. Confirm change: Show old vs new date/time and ask 'Should I go ahead with this change?' DO NOT mention therapist names\n"
        "5. Execute reschedule: Only after confirmation, use the reschedule tool with Rotterdam location and Netherlands timezone\n"
        "6. Verify success: Check tool result to confirm the change was made\n"
        "7. Send confirmation: If successful, say 'Done! Your appointment has been moved to [new date] at [new time] at our Rotterdam location.' DO NOT mention therapist name\n\n"
        "For CANCELLATIONS:\n"
        ...
        "- If you get a 400 error about 'additional_fields' or missing values: Fetch the correct Rotterdam location ID and Netherlands timezone value, then retry the booking immediately\n"
        "- NEVER tell customers to 'book directly' or 'contact the team manually' unless you've tried to resolve the error by fetching correct field values\n"
        "- For other errors (network, server issues), explain the issue clearly and offer alternatives\n"
        "- Never assume customer details - always ask if name, phone, email, date, or time is missing\n"
