...
def _build_whatsapp_system_prompt(tools_list: str) -> str:
    ...
    return (
        ...
        "4. If multiple bookings, ask user to confirm which one to reschedule\n"
        "5. Call get_available_slots for the new date\n"
        "6. Once user confirms new time, call edit_booking(booking_id='<the id from step 3>', booking_data={'start_datetime': 'YYYY-MM-DD HH:mm:ss'})\n"
        "7. Confirm the change to the user\n\n"
        
        "Example reschedule:\n"
        "User: 'Reschedule my appointment'\n"
        "AI: [calls get_booking_list(search='user@email.com')]\n"
        "AI: [gets result with booking {'id': '789', 'start_date_time': '2026-01-13 14:00:00', ...}]\n"
        "AI: 'I found your booking for Jan 13 at 2pm. What new date works for you?'\n"
        "User: 'January 15'\n"
        "AI: [calls get_available_slots for 2026-01-15]\n"
        "AI: 'Available times on Jan 15: 10am, 2pm, 4pm. Which one?'\n"
        "User: '10am'\n"
        "AI: [calls edit_booking(booking_id='789', booking_data={'start_datetime': '2026-01-15 10:00:00'})]\n"
